...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.down2 = UNetDown(3, 128, normalize=False)
...
class Decoder(nn.Module):
    def __init__(self, noise_dim):
        ...
        final = [   nn.ConvTranspose2d(256, 3, 4, stride=2, padding=1),
                    nn.Tanh() ]
        self.final = nn.Sequential(*final)     

    def forward(self, c_z, feats):
        [d2, d3, d4, d5, d6] = feats
        u1 = self.up1(c_z, d6)
        u2 = self.up2(u1, d5)
        u3 = self.up3(u2, d4)
        u4 = self.up4(u3, d3)
...
class PSPDiscriminator(nn.Module):
    def __init__(self, d=128):
        ...
        self.conv1 = SpectralNorm(nn.Conv2d(3, 32, 4, 2, 1))
        self.conv2 = SpectralNorm(nn.Conv2d(32, 64, 4, 2, 1))

        # pyramid conv
        self.pool = nn.MaxPool2d(2, 2)
        self.conv_16_1 = SpectralNorm(nn.Conv2d(64, 32, 3, 1, 1))
        self.conv_16_2 = SpectralNorm(nn.Conv2d(32, 16, 3, 1, 1))
        self.conv_8_1 = SpectralNorm(nn.Conv2d(64, 32, 3, 1, 1))
        self.conv_8_2 = SpectralNorm(nn.Conv2d(32, 16, 3, 1, 1))
        self.conv_4_1 = SpectralNorm(nn.Conv2d(64, 32, 3, 1, 1))
        self.conv_4_2 = SpectralNorm(nn.Conv2d(32, 16, 3, 1, 1))
        self.conv_2_1 = SpectralNorm(nn.Conv2d(64, 32, 3, 1, 1))
        self.conv_2_2 = SpectralNorm(nn.Conv2d(32, 16, 3, 1, 1))

        # final conv
        self.conv3 = SpectralNorm(nn.Conv2d(128, 256, 4, 2, 1))
        self.conv4 = SpectralNorm(nn.Conv2d(256, 512, 4, 2, 1))
        self.conv5 = SpectralNorm(nn.Conv2d(512, 1024, 4, 2, 1))
        self.conv6 = SpectralNorm(nn.Conv2d(1024, 1024, 2, 1, 0))
        self.conv7 = SpectralNorm(nn.Conv2d(1024, 1, 1, 1, 0))
    ...
    def forward(self, input):
        ...
        down_16 = F.upsample(down_16, (16, 16))
        down_8 = F.upsample(down_8, (16,16))
        down_4 = F.upsample(down_4, (16,16))
        down_2 = F.upsample(down_2, (16,16))
